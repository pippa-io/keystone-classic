...
module.exports = function openDatabaseConnection (callback) {
	...
	if (keystone.get('mongo replica set')) {
		...
		console.log('HEINN', keystone.get('mongo'), keystone.get('mongo options'));
		console.log('keystone.mongoose.connect', keystone.mongoose.connect.toString());
		keystone.mongoose.connect(keystone.get('mongo'), keystone.get('mongo options'))
		.then(
			() => { console.log('HEIN promise connected');},
			err => {
				console.log('HEIN promise ERROR', err);
			})
	}
